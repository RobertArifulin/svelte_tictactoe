<script lang="ts">
	...
	import { makeGameStore, wasAlerted, } from '$lib/gameStore';
	import Game from '$lib/Game.svelte';
	import { loginStore } from '$lib/loginStore';
	import { bubble } from 'svelte/internal';

	export let data;
	console.log(data.id, $loginStore)
	let gameStore = undefined;
	$: if ($gameStore === undefined && $loginStore.token != "-1" && $loginStore.token != undefined)  {
		gameStore = makeGameStore(data.id, $loginStore);
	}
...
<div class="main">
	{#if $gameStore}
		...
		<div class="indiv">
			...
			{#if ($gameStore.game.winner != null | $gameStore.game.draw)}
				{#if $gameStore.game.draw}
					<p1 class="text">Результат игры: Ничья</p1>
				{:else}
					<p1 class="text">Результат игры: Победили {$gameStore.game.winner}</p1>
				{/if}
			{:else}
				<p1 class="text">Ход: {$gameStore.game.currentPlayer}</p1>
			{/if}
		</div>
		<br />
		<div class="indiv">
			<Game gameData={$gameStore.game} {scrSize} on:turn={onTurn} />
		</div>
		{:else}
